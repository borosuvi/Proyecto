...
###Ventanas Emergentes
def infoAdicional():
    # messagebox.showinfo("Información","Versión: BETA")
    ...
    valor=messagebox.askokcancel("Atención","Desea salir de la palicación")
    print(valor)
...
menuCrud.add_command(label="Actualiza")
...
labelTitulo.config(fg="black",bd=2,font=("Arial",13))
labelTitulo.pack()
...
parword.grid(row=4,column=1,pady=5)
parword.config(show="*")

#Area de texto
comentario=Text(miFrame,width=15,height=5)
comentario.grid(row=5,column=1,pady=5)
#Boton
def codigoBoton():
    print(minombre.get())
